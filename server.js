...
async function emitSpectralReceipt({ event_type, did, amount_usd, tool_name, tx_hash, metadata }) {
  ...
  try {
    ...
    console.warn('[credit] receipt emit failed (non-fatal):', _.message || _);
  }
}
...
app.post('/v1/subscription', async (req, res) => {
  ...
  if (tier !== 'enterprise' && !tx_hash) {
    return res.status(402).json({
      ...
      x402: {
        type: 'x402', version: '1', kind: 'subscription_credit',
  ...
  await emitSpectralReceipt({
    ...
    tx_hash: tx_hash || null,
    metadata: { tier, service: 'Hive Credit', expires_ms: record.expires_ms },
  });
  ...
  return res.json({
    ...
    partner_attribution: 'Credit scoring and attestation — Hive scores, lenders decide. Partners: DeFi lending protocols',
    brand: '#C08D23',
